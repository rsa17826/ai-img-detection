...
def stopCapture():
  ...
  if gamecapidx:
    log("stopping gamecapidx")
    gamecapidx = None # Release the camera resource
...
def quitApp():
  global gamecap
  if gamecap:
    gamecap.release() # Release the camera resource if it's open
...
# Expose a function to start capturing video from the specified camera
@eel.expose
def startCaptureGame(idx):
  global gamecap, gamecapidx
  idx = int(idx) # Convert the input index to an integer
  log(f"gamecapidx Attempting to start capture on camera index: {idx}")
  gamecapidx = idx # Set the camera index to the global variable
  gamecap = cv2.VideoCapture(idx) # Initialize the VideoCapture object
  if not gamecap.isOpened():
    log(
      f"gamecapidx Failed to open camera with index {idx}. Please check the index and try again."
    ) # Log error if camera fails to open
  else:
    log(
      f"gamecapidx camera with index {idx} was successfully opened"
    ) # Log success
...
def reset():
  global lastFace, deathBoxList, speed, score, deathPosRand, spawnNewDeathRand, stopped, highScore, size, dirRand
  lastFace = None
  deathBoxList = []
  speed = 5
  gameScore = 0
        ...
        if name:
          ...
          if not collision:
            if name not in gameScores:
              gameScores[name] = 0
            gameScores[name] += int(facePos[3] / 3)
